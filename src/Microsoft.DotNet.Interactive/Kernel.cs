...
namespace Microsoft.DotNet.Interactive
{
    public abstract partial class Kernel : IDisposable
    {
        ...
        private readonly Subject<UserKernelMessage> _userMessages = new Subject<UserKernelMessage>();
        ...
        // TODO: not clear if we still need this when events are all published as messages.
        ...
        public Task SendMessage<T>(string label, T message)
        {
            ...
            _kernelMessages.OnNext(new UserKernelMessage($"user/{label}", JToken.FromObject(message, serializer)));
            return Task.CompletedTask;
        }

        public Task ReceiveUserMessage(UserKernelMessage message)
        {
        ...
        private IDisposable SubscribeWithFilter<T>(Func<string, bool> filter, IObserver<T> observer)
        {
            return ParentKernel != null
                ...
                : _userMessages
                    ...
                    .Select(m => m.Payload.ToObject<T>())   // TODO: serialization settings?
                    .Subscribe(observer);
        }

        public IDisposable SubscribeToMessagesWithLabelPrefix<T>(string label, IObserver<T> observer)
        {
        ...
        {
            string prefix = $"user/{label}";
            return SubscribeWithFilter(label => label.Equals(prefix, StringComparison.Ordinal), observer);
        }
